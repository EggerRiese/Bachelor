...
std::vector<std::vector<std::tuple<float,float>>> orderedCoordinates;
std::vector<vector<double>> cc;

std::vector<std::vector<std::tuple<float,float>>> getContourCoordinates(){
    std::vector<std::tuple<float,float>> _temp;
    //cc.push_back(yCoordinaten);
    std::sort(begin(coordinates), end(coordinates));
    for(int i = 0 ; i < coordinates.size() -1; i++){
        if(i == 0){
            _temp.push_back(coordinates[i]);
        }
        if(std::get<0>(coordinates[i]) == std::get<0>(coordinates[i+1])){
            _temp.push_back(coordinates[i+1]);
        }else {
            orderedCoordinates.push_back(_temp);
            _temp.clear();
            _temp.push_back(coordinates[i+1]);
        }
    }
...
public:
    ...
    void step(int stepNr)
    {
        // Balloon
        if(_balloon > 0)
        {
            dilate(_narrowBand, [&](const Position<D>& pos){
                    if(stepNr == 109){                    
                    coordinates.push_back(std::make_tuple(pos.coord[1], pos.coord[0]));
                }
                    return _image[pos.coord] > _threshold / _balloon;});
        }
        else if(_balloon < 0)
        {
            erode(_narrowBand, [&](const Position<D>& pos)
            {
                if(stepNr == 109){                    
                    coordinates.push_back(std::make_tuple(pos.coord[1], pos.coord[0]));
                }
